...
export default function MovieCard({ movie, onClick, isFavorite, onFavoriteToggle, index = 0 }: MovieCardProps) {
  ...
  return (
    <div
      className="bg-surface rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 ease-out hover:-translate-y-1 hover:border-primary/40 hover:shadow-2xl group relative border border-rim animate-fade-in-up"
      onClick={onClick}
      role="button"
      tabIndex={0}
    ...
    >
      <div className="relative aspect-[2/3] bg-surface">
        ...
        <button
          ...
          className="absolute top-2 right-2 w-7 h-7 rounded-full bg-black/50 backdrop-blur-sm flex items-center justify-center transition-colors duration-150 hover:bg-black/70"
