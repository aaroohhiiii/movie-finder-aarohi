...
export default function MovieCard({ movie, onClick, isFavorite, onFavoriteToggle, index = 0 }: MovieCardProps) {
  ...
  return (
    <div
      className="bg-surface rounded-2xl overflow-hidden cursor-pointer transition-all duration-300 ease-out hover:-translate-y-1 hover:border-primary/40 hover:shadow-2xl group relative border border-[#1E1E2A] animate-fade-in-up h-full flex flex-col"
      onClick={onClick}
      role="button"
      tabIndex={0}
    ...
    >
      <div className="relative w-full aspect-[2/3] bg-surface flex-shrink-0">
        {posterUrl ? (
          <>
            <img
              src={posterUrl}
